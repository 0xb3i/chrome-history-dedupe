...
- 弹窗保持以 tab 为单位的全局排序，用位置一致的来源标签和稳定色点加强分组信号；不要为了让同组连续展示而改回域名层次排序，也不要默认加入独立的“最相关”分区。
- 默认列表控制信息密度：分组身份应清晰，参数差异等解释性信息优先放入展开区；调整分组感时不额外堆叠说明行。

## 验证与交付

- 修改扩展代码后运行 `npm run verify`，默认仅做自动化测试和静态检查。
- 不再默认执行真实浏览器 smoke test、手动交互验收、截图检查或为验收搭建隔离浏览器页面；只有用户明确要求时才执行浏览器测试。
- 不因缺少浏览器测试、开发者模式未开启或验证 Profile 中扩展禁用而阻塞交付，也不要求用户为测试操作浏览器设置。
- 本项目无需构建，Chrome 直接把仓库根目录作为 unpacked extension 加载。不要添加伪 `test:browser` 脚本或硬编码扩展 ID。
- 修改扩展功能后同步更新用户可见版本号。无法自动重载用户正在使用的扩展时，简要提示用户自行重载即可，无需再启动浏览器验收流程。
